...
uint8_t init_plugboard(struct Plugboard *plugboard, struct Ui *ui, uint8_t *sign)
{
    ...
    while (p->next)
    {
        ...
        for (uint8_t i = 0; temp; i++)
        {
            ...
            case 4:
                if (!strcmp(temp, sign))
                {
                    plugboard->connect[index++] = ui->ch - 'A';
                }
                break;
            }       
            temp = strtok(NULL, " ");
...
uint8_t output_plugboard(struct Plugboard *plugboard, uint8_t input)
{
    if (input >= NUMS) return 0xff;
    return plugboard->connect[input];
}